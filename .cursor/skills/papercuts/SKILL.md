...
- **Local state you corrupted.** A stale dev-server port, a dirty cache, a
  leftover `tui.test` binary from `go test -c`. Re-run the cleanup.
...
1. Search `.cursor/PAPERCUTS.md` for an equivalent entry and avoid duplicates.
...
Use UTC timestamps and a short agent label (`cursor`, `human`). Add a PR or task
identifier only when it helps future triage.
...
5. Move fixed items to `## Resolved`, check them, and append the resolving date
   or commit. Route real bugs to normal issue/fix work.
