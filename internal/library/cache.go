...
// Missing waveforms do not count; those are warmed in the background.
...
// MissingWaveforms returns indexed tracks that have no waveform cache file.
func MissingWaveforms(env Env) ([]string, error) {
	paths, err := ListTrackPaths(env)
	if err != nil {
		return nil, err
	}
	out := make([]string, 0)
	for _, path := range paths {
		if waveformCacheFind(env, path) == "" {
			out = append(out, path)
		}
	}
	return out, nil
