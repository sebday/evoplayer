...
var genreTagToFolder = map[string]string{
	"drum & bass":   "drum&bass",
...
func genreFolderFromTags(path string, tag tags.TagInfo) string {
	...
	for _, field := range []string{tag.Genre, tag.Artist, tag.Title} {
		if folder := resolveGenreFolder(field); folder != "" {
			return folder
		}
	}
	return ""
...
func resolveGenreFolder(name string) string {
	lower := strings.ToLower(strings.TrimSpace(name))
	if lower == "" {
		return ""
	}
	if folder, ok := genreTagToFolder[lower]; ok {
		return folder
	}
	return ""
}
